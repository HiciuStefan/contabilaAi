...
class MatchingServiceTest(unittest.TestCase):
    ...
    def test_matching_service_supports_invoice_paid_in_multiple_installments(self) -> None:
        db_path = ROOT / "test_matching_installments.sqlite3"
        if db_path.exists():
            db_path.unlink()
        try:
            store = SQLiteTransactionStore(db_path)
            ...
            store.insert_many(
                [
                    ...
                    ImportedTransaction(
                        ...
                        description="Plata diferenta factura Casa Decor",
                        amount=-250.0,
                        currency="RON",
                        balance=2950.0,
                        merchant="Casa Decor SRL",
                        source_file="statement.csv",
                        raw_payload='{"id":"tx-250"}',
                    ),
                ],
                workspace_id=workspace_id,
            )
            import_batch_id = store.create_document_import_batch(
                source_path=ROOT / "_received_installments.json",
                workspace_id=workspace_id,
                source_type="received_invoice",
            )
            store.insert_invoices(
            ...
            self.assertEqual(proposals[0]["matched_amount"], 500.0)
            self.assertEqual(proposals[0]["residual_amount"], 250.0)
            self.assertEqual(proposals[1]["match_kind"], "installment_payment")
            self.assertEqual(proposals[1]["matched_amount"], 250.0)
            self.assertEqual(proposals[1]["residual_amount"], 0.0)
        finally:
            if db_path.exists():
                db_path.unlink()

    def test_matching_service_splits_single_payment_across_two_received_invoices(self) -> None:
        db_path = ROOT / "test_matching_bulk_settlement.sqlite3"
        if db_path.exists():
            db_path.unlink()
        try:
            store = SQLiteTransactionStore(db_path)
            ...
            store.insert_many(
                [
                    ImportedTransaction(
                        ...
                        description="Plata multiple facturi Casa Decor",
                        amount=-1000.0,
                        currency="RON",
                        balance=2200.0,
                        merchant="Casa Decor SRL",
                        source_file="statement.csv",
                        raw_payload='{"id":"tx-1000-split"}',
                    )
                ],
                workspace_id=workspace_id,
            )
            import_batch_id = store.create_document_import_batch(
                source_path=ROOT / "_received_bulk_settlement.json",
                workspace_id=workspace_id,
                source_type="received_invoice",
            )
            store.insert_invoices(
            ...
            self.assertEqual(len(proposals), 2)
            self.assertEqual([proposal["match_kind"] for proposal in proposals], ["bulk_settlement", "bulk_settlement"])
            self.assertEqual([proposal["matched_amount"] for proposal in proposals], [400.0, 600.0])
            self.assertEqual([proposal["residual_amount"] for proposal in proposals], [0.0, 0.0])
        finally:
            if db_path.exists():
                db_path.unlink()
